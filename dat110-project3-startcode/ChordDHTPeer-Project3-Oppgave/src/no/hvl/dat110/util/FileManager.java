...
public class FileManager {
	...
	public void createReplicaFiles() {
		...
		for (int i = 0; Util.numReplicas > i; i++) {

			String fileID = filename + i;
			// hash the replica
			BigInteger hash = Hash.hashOf(fileID);
	...
	public int distributeReplicastoPeers() throws RemoteException {
		...
		// target.saveFileContent(filename, fileID, bytesOfFile, primary);
		// create replicas of the filename
		createReplicaFiles(); // Inserted all replicas into array replicafiles.
		// for each replica, find its successor by performing findSuccessor(replica)
		for (BigInteger f : replicafiles) {
			NodeInterface succ = chordnode.findSuccessor(f);
			if (succ != null) {
				succ.addKey(f);
				if (counter == index) {
					succ.saveFileContent(filename, f, bytesOfFile, true);

				} else {
					succ.saveFileContent(filename, f, bytesOfFile, false);
				}
				// increment counter
				counter++;
			}
	...
	public Set<Message> requestActiveNodesForFile(String filename) throws RemoteException {
		...
		for (BigInteger f : replicafiles) {
			NodeInterface succ = chordnode.findSuccessor(f);
			Message m = succ.getFilesMetadata().get(f);
			succinfo.add(m);
		}
