...
public class MutualExclusion {
	...
	public boolean doMutexRequest(Message message, byte[] updates) throws RemoteException {
		...
		List <Message> activePeers = removeDuplicatePeersBeforeVoting();
		// multicast the message to activenodes (hint: use multicastMessage)
		multicastMessage(message, activePeers);
		// check that all replicas have replied (permission)
		if (queueack.size()==Util.numReplicas) {
			acquireLock();
	...
	public void onMutexRequestReceived(Message message) throws RemoteException {
		...
		// if message is from self, acknowledge, and call onMutexAcknowledgementReceived()
			if (message.getNodeID().equals(node.getMessage().getNodeID())) {
		...
		switch(condition) {
			...
			case 2: {
				...
				if (node.getMessage().getClock() > message.getClock() ) {
					...
					NodeInterface stub = Util.getProcessStub(procName, port);
					stub.onMutexAcknowledgementReceived(message);
				}else if (node.getMessage().getClock() == sender) {
