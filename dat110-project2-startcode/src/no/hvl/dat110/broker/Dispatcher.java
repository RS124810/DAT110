...
public class Dispatcher extends Stopable {
	...
	//	storage.getSubscribers(topic).forEach(x -> storage.getSession(x).send(msg));		
		
		Set<String> a = storage.getSubscribers(topic);
		
	//	a.iterator().forEachRemaining(x -> storage.getSession(x).send(msg));
		
		for (String user : a) {
			ClientSession session = storage.getSession(user);
			System.out.print("her1"+ user);
			if (session != null) {
				session.send(msg);
